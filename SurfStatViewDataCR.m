function [ a, cb ] = SurfStatViewDataCR( data, surf, title, background, cmap )
...
trisurf(surf.tri(tr,:)-cuv,surf.coord(1,vr),surf.coord(2,vr),surf.coord(3,vr),...
    double(data(vr)),'EdgeColor','none','AmbientStrength',0.5, 'SpecularStrength', 0.1);
view(90,0);
daspect([1 1 1]); axis tight; camlight; axis vis3d off;
%lighting flat; material dull; shading interp;


a(4)=axes('position', [0.62 0.55 h*3/4 w] );
trisurf(surf.tri(tl,:),surf.coord(1,vl),surf.coord(2,vl),surf.coord(3,vl),...
    double(data(vl)),'EdgeColor','none','AmbientStrength',0.5, 'SpecularStrength', 0.1);
view(90,0); 
daspect([1 1 1]); axis tight; camlight; axis vis3d off;
...
a(5)=axes('position',[0.62 0.1 h*3/4 w]);
trisurf(surf.tri(tr,:)-cuv,surf.coord(1,vr),surf.coord(2,vr),surf.coord(3,vr),...
    double(data(vr)),'EdgeColor','none','AmbientStrength',0.5, 'SpecularStrength', 0.1);
view(-90,0);
daspect([1 1 1]); axis tight; camlight; axis vis3d off;
